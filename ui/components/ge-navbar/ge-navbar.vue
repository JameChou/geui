<template>
	<ge-sticky minus-nav-height :disabled="!sticky" :zIndex="zIndex">
		<view class="ui-navbar" :style="[{minHeight: height > 0 ? `${height}px` : `${system_navbar_height}px`}]"
			:id="`nav_${uuid}`">
			...
			<!-- #endif -->
			<view class="ui-navbar-header"
				:style="[{height: height > 0 ? `${height}px` : `${system_capsule.height}px`}]">
