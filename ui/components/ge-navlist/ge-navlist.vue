<template>
  <scroll-view scroll-x scroll-with-animation class="ui-nav-list" :class="type === 'button' ? '' : 'regular'" :scroll-left="scrollLeft">
    <view v-for="(item, index) in items" :data-id="index"  @tap="tabSelect"
          :class="[activeIndex === index ? 'active' : '', type === 'button' ? ' btn-item' : ' regular-item']">{{item}}</view>
  </scroll-view>
</template>
<script>
export default {
